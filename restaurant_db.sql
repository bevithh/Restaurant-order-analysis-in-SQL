...
SELECT category, COUNT(order_id) AS num_items FROM order_details od LEFT JOIN menu_items mi
ON od.item_id = mi.menu_item_id
WHERE order_id = 440
GROUP BY category;
...
SELECT category, order_id, COUNT(order_id) AS num_items FROM order_details od LEFT JOIN menu_items mi
ON od.item_id = mi.menu_item_id
